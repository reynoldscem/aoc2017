...
def main():
    ...
    # To track if we're in a cycle, and the index of when we
    #  first saw a state.
    states = set()
    states_index = {}
    ...
    for cycle_index in count(1):
        ...
        #  if not add it in, and record indices.
        frozen_state = tuple(banks)
        if frozen_state in states:
            break
        states.add(frozen_state)
        states_index[frozen_state] = cycle_index

    # Output the difference between the last iteration (when we broke)
    #  and the index when we first saw the state.
    print(cycle_index - states_index[frozen_state])
