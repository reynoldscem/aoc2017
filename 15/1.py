...
def make_generator(seed_value, factor):
    modulo_val = 2**31 - 1
    value = seed_value
    while True:
        value *= factor
...
def main():
    ...
    generator_a = make_generator(generator_a_seed, 16807)
    generator_b = make_generator(generator_b_seed, 48271)

    combined_generator = zip(generator_a, generator_b)

    equal_generator = (a == b for a, b in combined_generator)
    truncated_generator = islice(equal_generator, 4 * 10**7)
